...
long ntp_writeCalback(char* tx, uint16_t tx_len, uint16_t socket_Position);
//=============================================================================
...
void ntp_read_callback(char* rx, uint16_t rx_len, sockaddr* from,
	...
	uart_send("=> RTC aktualisiert\r\n");
	handle_NTP = CC3000_closeSocket(handle_NTP);
	memset(&rx_daten, 0, sizeof(rx_daten));
	S3_WaitForAnswer = false;
	RTC_AlarmCmd(RTC_Alarm_A, DISABLE);
//	Konfiguration.letzerNTPsync = rtc_getSek(TIMEBASE_1900);
//	if (iot_dev.startZeit == 0) {
//		iot_dev.startZeit = rtc_getSek(TIMEBASE_1900);
//	}
...
unsigned long get_NTP_Time(char* SNTP_Server_Name) {
	...
	if (index++ > 9)
		index = 0;
	sprintf(out, "=> NTP Zeitserver: %s\r\n", NTP_Servers[index]);
	uart_send(out);

	if (CC3000_DHCP_done == true) { // Verbindung ist aufgebaut

		gethostbyname(NTP_Servers[index], strlen(NTP_Servers[index]),
				&NTP_Server);
		...
		handle_NTP = CC3000_openSocketul(NTP_Server, port_NTP, false, false,
				false, false, ntp_read_callback, ntp_writeCalback, 0);
		S3_WaitForAnswer = false;
		memset(tx_buf[handle_NTP], 0, sizeof(2048));
		memset(rx_buf[handle_NTP], 0, sizeof(2048));
		uart_send("=> NTP Paket versenden\r\n");
		...
		result = sendto(handle_NTP, &tx_buf[handle_NTP], 48, 0, &sockets[handle_NTP],
				sizeof(S3_NTP));

		if (result >= 0) {
			uart_send("=> NTP Paket abgeschickt\r\n");
			S3_WaitForAnswer = true;
			set_RTC_Alarm_in(0, 0, 0, 20, &retrieve_NTP_time_Callback);
			start_RTC_Alarm();
		}
	}
	return handle_NTP;
